...
void read_data (char *file_name, struct info_param *param, struct info_chips **chips, int ***chip_coord)
{
  ...
  fscanf (fdin, "%d %d %f %f %f %d", &param->scale, &param->nchip, &param->t_ext, &param->tmax_chip, &param->t_delta, &param->max_iter);
  if (param->scale > 12) {
  ...
  *chip_coord = (int **) malloc (sizeof(int*));
  ...
  for (j=0; j<param->nchip; j++)
    fscanf (fdin, "%d %d", &(*chip_coord)[0][2*j], &(*chip_coord)[0][2*j+1]);
